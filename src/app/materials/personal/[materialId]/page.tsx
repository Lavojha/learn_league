...
export default async function PersonalMaterialPage({ params }: { params: Promise<{ materialId: string }> }) {
  ...
  return <main className="mx-auto min-h-screen max-w-3xl px-6 py-10"><Link href="/materials/personal" className="muted text-sm">← My materials</Link><section className="card mt-6 p-7"><p className="text-sm text-violet-400">Personal PDF</p><h1 className="mt-2 text-3xl font-bold">{material.title}</h1><p className="muted mt-3">{material.description || material.originalFileName}</p><div className="mt-7 flex gap-3"><Link href={`/materials/personal/${materialId}/viewer`} className="rounded-xl bg-violet-500 px-5 py-3 font-semibold">Open PDF</Link></div></section></main>;
}
