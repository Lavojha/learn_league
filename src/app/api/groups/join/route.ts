...
export async function POST(request: Request) {
  try {
    ...
    if (body.inviteCode) {
      ...
      [group] = await db.select().from(groups).where(eq(groups.inviteCode, code)).limit(1);
    } else {
    ...
    const [existing] = await db.select().from(groupMembers).where(and(eq(groupMembers.groupId, group.id), eq(groupMembers.userId, user.id), eq(groupMembers.status, "active"))).limit(1);
    if (existing) return Response.json({ success: true, status: "already_member" });
