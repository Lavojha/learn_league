...
export async function PATCH(request: Request, { params }: { params: Promise<{ groupId: string }> }) {
  try {
    ...
    if (!canManageRole(actor.role, target.role) || !canAssignRole(actor.role, body.role) || !isGroupRole(body.role)) return Response.json({ error: "You cannot change this member's role" }, { status: 403 });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ groupId: string }> }) {
  try {
    ...
    await db.update(groupMembers).set({ status: "removed", updatedAt: new Date() }).where(eq(groupMembers.id, target.id));
