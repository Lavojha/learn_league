...
export async function GET() {
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load study sessions" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (startedAt > now) return Response.json({ error: "Study session cannot start in the future" }, { status: 400 });
    ...
    if (endedAt > now) return Response.json({ error: "Study session cannot end in the future" }, { status: 400 });
