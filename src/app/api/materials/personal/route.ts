...
export async function POST(request: Request) {
  try {
    ...
    const [material] = await db.insert(personalMaterials).values({ userId: user.id, title, description, storageKey, originalFileName: file.name, mimeType: file.type, fileSizeBytes: file.size }).returning();
