...
export async function PATCH(request: Request, { params }: { params: Promise<{ materialId: string }> }) {
  try {
    ...
    if (title === undefined && description === undefined) return Response.json({ error: "Nothing to update" }, { status: 400 });

    const [material] = await db.update(personalMaterials).set({ ...(title !== undefined ? { title } : {}), ...(description !== undefined ? { description } : {}), updatedAt: new Date() }).where(and(eq(personalMaterials.id, materialId), eq(personalMaterials.userId, user.id))).returning();
