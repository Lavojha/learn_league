...
export async function POST(request: Request, { params }: { params: Promise<{ materialId: string }> }) {
  try {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes("duplicate key")) return Response.json({ error: "This material is already open in another active session" }, { status: 409 });
      throw error;
    }
  } catch (error) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ materialId: string }> }) {
  try {
    ...
    if (action === "end") {
      if (!["active", "paused"].includes(session.status)) return Response.json({ error: "Session is already closed" }, { status: 400 });
      const [ended] = await db.update(materialAccessSessions).set({ status: "ended", endedAt: new Date() }).where(and(eq(materialAccessSessions.id, session.id), inArray(materialAccessSessions.status, ["active", "paused"]))).returning();
      return ended ? Response.json({ success: true, session: ended }) : Response.json({ error: "Session changed; please refresh" }, { status: 409 });
    }
    ...
    if (material.availableUntil && now >= material.availableUntil) {
      await db.update(materialAccessSessions).set({ status: "expired", endedAt: now }).where(and(eq(materialAccessSessions.id, session.id), inArray(materialAccessSessions.status, ["active", "paused"])));
    ...
    if (action === "pause") {
      ...
      if (now >= session.expiresAt) {
        const [expired] = await db.update(materialAccessSessions).set({ status: "expired", endedAt: now }).where(and(eq(materialAccessSessions.id, session.id), eq(materialAccessSessions.status, "active"))).returning();
        return Response.json({ error: "Material access has expired", session: expired });
      }
      if (!material.allowPause) return Response.json({ error: "Pause is disabled for this material" }, { status: 403 });
      const [paused] = await db.update(materialAccessSessions).set({ status: "paused", pausedAt: now }).where(and(eq(materialAccessSessions.id, session.id), eq(materialAccessSessions.status, "active"))).returning();
      return paused ? Response.json({ success: true, session: paused }) : Response.json({ error: "Session changed; please refresh" }, { status: 409 });
    }
