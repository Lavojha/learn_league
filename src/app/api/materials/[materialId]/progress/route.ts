...
export async function GET(_: Request, { params }: { params: Promise<{ materialId: string }> }) {
  try {
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Unable to load progress" }, { status: 400 });
  }
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ materialId: string }> }) {
  try {
    ...
    materialIdSchema.parse(materialId);
    if (!(await canViewMaterial(user.id, materialId))) return Response.json({ error: "Material not found or inaccessible" }, { status: 404 });
    const [session] = await db.select({ id: materialAccessSessions.id, status: materialAccessSessions.status, expiresAt: materialAccessSessions.expiresAt }).from(materialAccessSessions).where(and(eq(materialAccessSessions.materialId, materialId), eq(materialAccessSessions.userId, user.id), inArray(materialAccessSessions.status, ["active", "paused"]))).limit(1);
    ...
    if (session.status === "active" && new Date() >= session.expiresAt) {
      await db.update(materialAccessSessions).set({ status: "expired", endedAt: new Date() }).where(and(eq(materialAccessSessions.id, session.id), eq(materialAccessSessions.status, "active")));
      return Response.json({ error: "Material access session has expired" }, { status: 403 });
    }
    ...
    const [progress] = await db.insert(materialProgress).values({ materialId, userId: user.id, progressPercent, totalStudySeconds, completed, lastOpenedAt: new Date(), completedAt }).onConflictDoUpdate({ target: [materialProgress.materialId, materialProgress.userId], set: { progressPercent, totalStudySeconds, completed, lastOpenedAt: new Date(), completedAt, updatedAt: new Date() } }).returning();
