...
export async function GET(_: Request, { params }: { params: Promise<{ materialId: string }> }) {
  ...
  if (new Date() >= session.expiresAt) return Response.json({ error: "Access session expired" }, { status: 403 });
  const signedUrl = await createSignedFileUrl(material.storageKey, false, 120);
  return Response.json({ signedUrl, expiresAt: session.expiresAt, allowPause: material.allowPause });
}
