...
export const materialExpiryActionSchema = z.enum([
  "remove_access",
  "enable_download",
  "archive",
]);
...
export const materialAvailabilitySchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    if (value.availabilityMode === "scheduled" && !value.availableFrom) {
      ctx.addIssue({
        code: "custom",
        path: ["availableFrom"],
        message: "Scheduled availability requires a start time",
      });
    }
    if (
      value.availableFrom &&
...
export const materialDownloadSettingsSchema = z
  .object({
  ...
  .superRefine((value, ctx) => {
    ...
    if (!value.downloadStartMode) {
      ctx.addIssue({
        code: "custom",
        path: ["downloadStartMode"],
        message: "Download start mode is required when downloads are enabled",
      });
    }
    if (value.downloadStartMode === "scheduled" && !value.downloadAvailableFrom) {
      ctx.addIssue({
        code: "custom",
        path: ["downloadAvailableFrom"],
        message: "Scheduled downloads require a start time",
      });
    }
    if (
      value.downloadAvailableFrom &&
      value.downloadAvailableUntil &&
      value.downloadAvailableUntil <= value.downloadAvailableFrom
    ) {
      ctx.addIssue({
        code: "custom",
        path: ["downloadAvailableUntil"],
        message: "Download end must be after start",
      });
    }
  });
