...
const columnByPermission: Record<
  GroupPermission,
  keyof typeof groupPermissions.$inferSelect
> = {
  manageMembers: "manageMembers",
  manageMaterials: "manageMaterials",
  manageGroupInfo: "manageGroupInfo",
...
export async function getRolePermissions(groupId: string, role: GroupRole) {
  const [stored] = await db
    ...
    .where(and(eq(groupPermissions.groupId, groupId), eq(groupPermissions.role, role)))
    .limit(1);
