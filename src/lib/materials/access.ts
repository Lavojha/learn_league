...
export async function canStartMaterialAccess(
  userId: string,
  materialId: string,
  now = new Date(),
) {
...
export async function getActiveMaterialSession(userId: string, materialId: string) {
  const [session] = await db
    ...
    .where(
      and(
        eq(materialAccessSessions.userId, userId),
        eq(materialAccessSessions.materialId, materialId),
        inArray(materialAccessSessions.status, ["active", "paused"]),
      ),
    )
    .orderBy(desc(materialAccessSessions.createdAt))
    .limit(1);
  return session ?? null;
}

export function calculateSessionExpiry(
  material: { accessDurationMinutes: number; availableUntil?: Date | null },
  startedAt: Date,
) {
  const sessionExpiry = new Date(
    startedAt.getTime() + material.accessDurationMinutes * 60_000,
  );
